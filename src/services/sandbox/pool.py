    ...
    async def _start_repl_process(
        ...
        try:
            ...
            wrapper_cmd = (
                ...
                # (soffice) for DOCX/XLSX/PPTX skills. The upstream comment
                # "REPL is Python-only, always safe to mask" doesn't hold for
                # this fork — soffice hard-fails without /proc. PID namespace
                # inside nsjail still restricts visibility to sandbox processes.
