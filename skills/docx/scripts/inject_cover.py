#!/usr/bin/env python3
"""Inject cover page into a pandoc-generated DOCX using a corporate template.

Usage:
    python3 inject_cover.py <input.docx> <output.docx> --title "..." [--subtitle "..."] [--author "..."] [--date "..."]

This script takes a DOCX generated by pandoc (with a reference doc) and:
1. Transplants 6 files from template-base.docx (styles, numbering, settings, theme, endnotes, footer)
2. Remaps pandoc style IDs (English) to template style IDs (French)
...
8. Injects the cover page (title/logo table + spacing + version table + page break)
9. Adds missing relationships (images, footer, endnotes) and content types
10. Packs and validates the output

The result is a fully branded document combining pandoc's robust markdown
parsing with the corporate template branding.
...
- Pandoc uses English style IDs (Heading1, BodyText) but template uses French (Titre1, Normal)
...
# Template directory: resolved from SKILLS_TEMPLATES_DOCX env var, or default path
_TEMPLATES_DIR = os.environ.get(
    "SKILLS_TEMPLATES_DOCX",
    str(SKILLS_ROOT / "templates" / "corporate"),
)
TEMPLATE_BASE = Path(_TEMPLATES_DIR) / "template-base.docx"
...
# Pandoc style ID → corporate template style ID mapping
PANDOC_STYLE_MAP = {
    # Headings
    "Title": "Titre",
    "Subtitle": "Sous-titre",
    ...
    # Code (pandoc source code styles → template code style)
    "SourceCode": "Code",
    "VerbatimChar": "CodeCar",
    # Pandoc syntax highlighting token styles (character styles)
    # These don't have template equivalents — remove outside code blocks
    "NormalTok": "CodeCar",
    "KeywordTok": "CodeCar",
    "DataTypeTok": "CodeCar",
    "DecValTok": "CodeCar",
...
def _build_version_table(date, author):
    ...
    """
    HEADER_FILL = "233F70"  # Corporate navy
...
def inject_cover(input_docx, output_docx, title, subtitle="", author="", date=""):
    ...
    try:
        ...
        # 6a. Remap pandoc style IDs to corporate template style IDs
        ...
        if remap_count:
            print(f"Remapped {remap_count} style reference(s) from pandoc IDs to template IDs")
...
def main():
    parser = argparse.ArgumentParser(description="Inject cover page into pandoc DOCX output using corporate template")
